...
def test_create_bucket_data_from_values():
    data = allocate.solvers.bucketdata.BucketData.from_values(values=[1, 2, 3, 4, 5])
...
def test_create_bucket_data_from_values_raises_on_negative_values():
    with pytest.raises(ValueError, match='negative values'):
        allocate.solvers.bucketdata.BucketData.from_values(values=[1, -1])


def test_create_bucket_data_from_ratios():
    data = allocate.solvers.bucketdata.BucketData.from_ratios(ratios=[1, 1, 1, 1], amount=1)
...
def test_create_bucket_data_from_ratios_raises_on_negative_ratios():
    with pytest.raises(ValueError, match='negative ratios'):
        allocate.solvers.bucketdata.BucketData.from_ratios(ratios=[1, -1], amount=1)
...
def test_create_bucket_system():
    system = allocate.solvers.bucketdata.BucketSystem.create(1, [0, 0], [0.5, 0.5])
